...
from CTFd.plugins.docker_challenges.config import *
...
class DockerChallengeTracker(db.Model):
    ...
    host = db.Column('host', db.String(1024), index=True)
    #host = db.Column('container_name', db.String(128), index=True)
    conn_type = db.Column('conn_type', db.String(10), index=True)
...
def scp_l2r(localpath, remotepath, user, passwd, instance):
    return subprocess.run(["sshpass", "-p", passwd, "scp", "-o", "StrictHostKeyChecking=no", localpath, f"{user}@{instance}:{remotepath}"]).returncode == 0

# run command through ssh
def ssh_cmd(user, passwd, instance, cmd):
    return subprocess.run(["sshpass", "-p", passwd, "ssh", f"{user}@{instance}", cmd]).returncode == 0
...
def update_haproxy(docker):
    prefix = ('http', 'https')[bool(docker.tls_enabled)]
    host = docker.hostname
    URL_TEMPLATE = '%s://%s' % (prefix, host)
    ...
    #containers = requests.get(url="%s/containers/json?all=1" % (URL_TEMPLATE)).json()
    ...
    with open(filename, 'w') as f:
        f.write(content)
    scp_l2r(filename, HAPROXY_CFG_RMT_PATH, USER, PASSWD, INTERNAL_INSTANCE_IP)
    ...
    with open(f"{HAPROXY_CFG_DIR}/{SNI_MAP_PATH}", 'w') as f:
        f.write(content)
    scp_l2r(filename, SNI_MAP_RMT_PATH, USER, PASSWD, INTERNAL_INSTANCE_IP)

    # upload the files to the docker VM
    ssh_cmd(USER, PASSWD, INTERNAL_INSTANCE_IP, 'sudo systemctl reload haproxy')
...
def create_container(docker, image, team, portbl):
    ...
    token = ''.join(random.choice(string.ascii_letters+string.digits) for _ in range(TOKEN_LENGTH)).lower()
    container_name = f"{token}-{image.split(':')[1]}"
    assigned_ports = dict()
    ...
    host = container_name + '.' + DOMAIN_NAME
    data = json.dumps({"host": host, "Image": image, "ExposedPorts": ports, "HostConfig": {"PortBindings": bindings, "Memory": MEMORY_LIMIT, "NanoCpus": NANO_CPUS_QUOTA}})
    if tls:
...
class ContainerAPI(Resource):
    ...
    def get(self):
        ...
        entry = DockerChallengeTracker(
            ...
            host=json.loads(create[1])['host'],
            conn_type=DockerChallenge.query.filter_by(docker_image=container).first().conn_type,
        )
    ...
    def get(self):
        ...
        for i in tracker:
            data.append({
                ...
                'ports': i.ports.split(','),
                'host': i.host,#str(docker.hostname).split(':')[0]
